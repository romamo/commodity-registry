...
class TestMapAssetClass:
    def test_etf_keyword(self):
        from instrument_registry.models import AssetClass
...
class TestResolveCurrency:
    ...
    def test_cross_rate(self):
        ...
        assert res is not None
        assert res.symbol.root == "EURJPY=X"
    ...
    def test_verify_calls_fetch_metadata(self):
        ...
        with patch(
            "instrument_registry.finder.fetch_metadata", return_value=MagicMock()
        ) as mock_fetch:
    ...
    def test_verify_returns_none_if_not_found(self):
        ...
        with patch("instrument_registry.finder.fetch_metadata", return_value=None):
            res = resolve_currency("EUR", verify=True)
            assert res is None
...
class TestGetAvailableProviders:
    def test_returns_yahoo_when_available(self):
        with patch("instrument_registry.finder.YFinanceDataSource", MagicMock()):
    ...
    def test_returns_ft_when_available(self):
        with patch("instrument_registry.finder.YFinanceDataSource", None):
            with patch("instrument_registry.finder.FTDataSource", MagicMock()):
                providers = get_available_providers()
...
class TestFetchPrice:
    def test_returns_price_when_get_price_exists(self):
        ...
        with patch("instrument_registry.finder.get_data_provider", return_value=mock_provider):
